...
local function createText()
    local memory = display.newText("00 00.00 000", 10, 0, "Maassslicer", 9)
    memory:setFillColor(1, .5)
    function memory:tap ()
...
function _M.labelUpdater(event)
    ...
    _M.text.text = " " .. tostring(mFloor( 1000 / (curTime - prevTime))) .. " FPS  " ..
            tostring(mFloor(sGetInfo("textureMemoryUsed") * 0.00001) * 0.1) .. " MB  " ..
